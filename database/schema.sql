...
CREATE TABLE vacation_plan (
	...
	destination_id int NOT NULL,
	status varchar NOT NULL DEFAULT 'Pending' CHECK (status in ('Pending','Scheduled','Completed'))
...
CREATE TABLE users_visibility (
        ...
        can_see_user int NOT NULL,
        CONSTRAINT PK_users_visibility PRIMARY KEY (user_current, can_see_user),
	CONSTRAINT FK_user_current FOREIGN KEY(user_current) REFERENCES users(user_id),
	CONSTRAINT FK_can_see_user FOREIGN KEY(can_see_user) REFERENCES users(user_id)
)
